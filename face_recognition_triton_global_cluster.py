...
class FaceReIDPipeline:
    ...
    def _save_all_best_faces(self):
        ...
        # ---------------- Collect best faces from the current video ----------------
        ...
        # ---------------- Prepare data from current video ----------------
        all_embeddings = []
        all_img_names = []
        all_crops = []
        ...
        # ---------------- Fetch historical embeddings from PostgreSQL ----------------
        ...
        # ---------------- Save only the current video batch to PostgreSQL ----------------
        self._insert_batch_to_pg(all_img_names[:len(authorized_best)+len(unauthorized_best)],
                                 all_crops[:len(authorized_best)+len(unauthorized_best)],
                                 all_embeddings[:len(authorized_best)+len(unauthorized_best)])
    ...
    def _insert_batch_to_pg(self, img_names, crops, embeddings):
        try:
            ...
            # Encode all crops as JPEG bytes
            crops_bytes = []
            for crop in crops:
                _, buffer = cv2.imencode(".jpg", crop)
            ...
            # print(self._fetch_face_clustering_batches())
    ...
    def _fetch_face_clustering_batches(self):
        try:
            ...
            for batch_id, img_names, crops_bytes, embeddings_list, created_at in rows:
                # Decode crops bytes back to images
