...
#         # keep legacy filename used previously; change if needed
...
#         for k in ["yolo_inference", "tracker_inference", "buffalo_processing", "faiss_search", "update_face", "save_face"]:
#             self.timing.setdefault(k, 0.0)

#     def _load_faiss_index(self, db_path):
#         # expects new format: (id, name, role, embedding)
#         with open(db_path, "rb") as f:
#             face_db_list = pickle.load(f)

#         # If older format (name, embedding) is present, this will raise; handle both
#         try:
#             ids, names, roles, embeddings = zip(*face_db_list)
#             names = np.array(names)
#             embeddings = np.stack(embeddings).astype("float32")
#         except Exception:
#             # fallback: (name, embedding)
...
#         # Option A: pass numpy crop directly (no base64)
#         if name == "No face":
#             return
#         if name == "Unknown":
...
#             self.best_face_per_id[tid] = (crop.copy(), name, conf, embedding.copy() if embedding is not None else None)
#         else:
#             # Prefer replacing Unknown with known person
#             if self.best_face_per_id[tid][1] == "Unknown":
#                 self.best_face_per_id[tid] = (crop.copy(), name, conf, embedding.copy() if embedding is not None else None)

#     def _save_all_best_faces(self):
#         """
#         Saves best authorized (known) and unauthorized (Unknown) faces to disk.
#         Returns a dictionary `save_best_faces` mapping saved_filename -> metadata dict
#         so the caller can include it in the final return.
#         """
...
#             filepath = os.path.join("pipeline_images", filename)
#             cv2.imwrite(filepath, crop)
#             print(f"✅ Saved best authorized crop for {name} | conf={conf:.2f}")
#             embeddings_list.append((filename, embedding))
#             save_best_faces[filename] = {"type": "authorized", "name": name, "conf": float(conf), "path": filepath}

#         # Save unauthorized crops (Unknown)
...
#             embeddings_list.append((filename, embedding))
#             save_best_faces[filename] = {"type": "unauthorized", "tid": int(tid), "conf": float(conf), "path": filepath}
...
#                     # update best face using raw numpy crop and embedding
#                     # We need the crop corresponding to this tid from this frame: recalc quickly
#                     # Note: face_crop used earlier is not returned by _process_crop_sync to avoid copying heavy arrays across threads.
#                     # Instead, we reconstruct crop from tracks for update using tid -> find track bbox in current 'tracks' list.
#                     # Find matching track bbox to extract the crop for _update_best_face
#                     matching_bbox = None
#                     for tr in tracks:
#                         if tr["track_id"] == tid:
#                             matching_bbox = tr["bbox"]
#                             break
...
#                     else:
#                         # If crop isn't available (should be rare), still try to store embedding in best_face_per_id if better
#                         if emb is not None:
#                             if tid not in self.best_face_per_id or (self.best_face_per_id[tid][1] == "Unknown"):
#                                 # store a placeholder black image if no crop available
...
#         # Automatically save all best faces (Option A chosen)
#         t0 = time.perf_counter()
#         save_best_faces = self._save_all_best_faces()
#         self.timing["save_face"] += time.perf_counter() - t0

#         # Save mappings to JSON (like Code 2 did)
...
class FaceReIDPipeline:
    ...
    def _save_all_best_faces(self):
        ...
        for lbl, members in cluster_map.items():
            ...
            filepath = os.path.join("pipeline_images", img_name)
            cv2.imwrite(filepath, crop)
